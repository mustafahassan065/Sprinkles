...
export const metadata = {
  title: 'Irrigation Blog | Sprinklers and Lawns Dallas Fort Worth',
  description: 'Irrigation tips, lawn care guides, and water-saving advice for North Texas homeowners from the experts at Sprinklers and Lawns.',
  alternates: { canonical: 'https://sprinklersandlawns.com/blog' },
};
      ...
      <section className="section bg-soft">
        <div className="container">
          <div style={{ display:'flex', gap:'8px', flexWrap:'wrap', marginBottom:'40px', alignItems:'center' }}>
            <span style={{ background:'var(--green)', color:'white', padding:'6px 16px', borderRadius:'20px', fontSize:'13px', fontWeight:600 }}>All Posts</span>
            {categories.map(cat => (
              <span key={cat} style={{ background:'white', border:'1px solid var(--gray-100)', color:'var(--text-muted)', padding:'6px 16px', borderRadius:'20px', fontSize:'13px', fontWeight:500 }}>{cat}</span>
            ))}
          </div>
            ...
            {posts.map(post => (
              <Link key={post.slug} href={`/blog/${post.slug}`} className="card"
                ...
                <div style={{ padding:'24px', flex:1, display:'flex', flexDirection:'column' }}>
                  ...
                  <h2 style={{ fontFamily:'var(--serif)', fontSize:'19px', fontWeight:400, color:'var(--text)', marginBottom:'10px', lineHeight:1.3 }}>{post.title}</h2>
